...
export class UserSessionUtils {
   ...
   static setUserDetails(userData) {
      const detailsToStore = {
        ...
        // Owner-related details (if available)
        ownerId: userData.ownerId || null,
        type: userData.entityType || null,
        status: userData.recordStatus || null
      };
   ...
   static setOwnerDetails(ownerData) {
      const detailsToStore = {
        ...
        status: ownerData.recordStatus,
        userId: ownerData.user?.id // Store related user ID
      };
      localStorage.setItem(
    ...
    static getOwnerDetails() {
      try {
         const data = sessionStorage.getItem(StorageParams.OWNER_DETAILS_JSON);
         return data ? JSON.parse(data) : null;
       } catch (error) {
